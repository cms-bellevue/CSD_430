 ...
 * Handles all database interactions including Insert, Select, 
...
public class MovieDAO {
     ...
     * Verifies Title, Director, and Year to allow for remakes (same title/director, different year)
     * while preventing exact duplicate entries.
     ...
     * Includes sanitization to ensure Genre is always Title Case (e.g., "Action").
    ...
    public void insertMovie(Movie movie) throws SQLException {
        ...
        // Casing Check: Sanitize Genre to "Title Case" (e.g., "sci-fi" -> "Sci-fi")
        // This ensures the dropdown list remains consistent and avoids duplicates like "Action" vs "action".
        String cleanGenre = movie.getGenre();
        if (cleanGenre != null && !cleanGenre.isEmpty()) {
            cleanGenre = cleanGenre.substring(0, 1).toUpperCase() + cleanGenre.substring(1).toLowerCase();
        }
        ...
        try (Connection conn = DriverManager.getConnection(url, username, getPassword());
            ...
            ps.setString(3, cleanGenre); // Insert the sanitized genre
     ...
     * Results are sorted by ID in descending order (Newest entries first).
     ...
     * Used to populate the Genre dropdown menu dynamically.
     ...
     * Used primarily for Module 6 selection logic.
